...
@section('title', 'Edit ' . $testimonial->name . ' Reviews')

@section('content')
    <section class="py-5">
    <div class="mb-4">
        <h5 class="title-heading d-inline-block float-left">Edit Testimonial</h5>
    ...
    @method('put')
     <div class="mb-6">
                ...
                <input type="file" name="profile_pic" value="{{ old('profile_pic') }}" placeholder="Profile picture"  id="" class="@error('name')border border-red-500 @enderror form-input mt-1 block w-full">
                </label>
                @error('profile_pic')
              ...
              <div class="mb-6">
                ...
                <input type="text" name="name" value="{{ old('name', $testimonial->name) }}" placeholder="Name"  id="" class="@error('name')border border-red-500 @enderror form-input mt-1 block w-full">
                </label>
                @error('name')
                     <p class="text-red-500 mt-1">{{ $message }}</p>
                @enderror
             ...
             <div class="mb-6">
                ...
                <input type="text" name="email" value="{{ old('email', $testimonial->email) }}" placeholder="Email"  id="" class="@error('email')border border-red-500 @enderror form-input mt-1 block w-full">
                </label>
                @error('email')
                     <p class="text-red-500 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                <label for="message" class="block">
                    ...
                    <textarea rows="3" id="editor" name="reviews" placeholder="Type here..."  id="" class="@error('reviews')border border-red-500 @enderror form-textarea mt-1 block w-full">
                        {{ old('reviews', $testimonial->reviews) }}
                    </textarea>
                </label>
                @error('reviews')
                    <p class="text-red-500 mt-1">{{ $message }}</p>
